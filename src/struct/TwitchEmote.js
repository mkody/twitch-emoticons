...
class TwitchEmote extends Emote {
     ...
     * @param {'light' | 'dark'} [backgroundColor] - The preferred background color. Defaults to the fetcher's twitchBackgroundColor or `dark`.
     * @returns {string}
     */
    toLink(size = 0, forceStatic, backgroundColor) {
        const background = backgroundColor || (this.channel && this.channel.fetcher ? this.channel.fetcher.twitchBackgroundColor : 'dark');
        return Constants.Twitch.CDN(this.id, size, forceStatic, background); // eslint-disable-line new-cap
    }
