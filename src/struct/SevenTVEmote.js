...
class SevenTVEmote extends Emote {
  ...
  _setup (data) {
    ...
    /**
     * If emote is NSFW/unlisted.
     * Do note that this flag isn't always applied to what *looks* NSFW.
     * @type {boolean}
     */
    this.nsfw = Boolean(data.emote.flags.nsfw || !data.emote.flags.publicListed)
  }
      ...
      emoteObject.id,
      {
        emote: {
          ...
          flags: {
            ...
            // We're lossy on how we store NSFW, since it's both from the `nsfw` and `publicListed` flags.
            // Applying the value to both here as best as we can, but it probably doesn't match reality.
            nsfw: emoteObject.nsfw || false,
            publicListed: !(emoteObject.nsfw || false),
          },
          images,
        },
      }
