...
describe('Test Twitch emotes', () => {
  ...
  if (env.TWITCH_ID === undefined || env.TWITCH_SECRET === undefined ||
    ...
    test.todo('Notice: Twitch client id/secret missing, not testing Helix fetching and parsing.')
  } else {
    describe('Test global emotes (Helix)', () => {
      const emoteFetcher = new EmoteFetcher({
    ...
    describe('Test user emotes (Helix)', () => {
      const emoteFetcher = new EmoteFetcher({
        twitchAppID: env.TWITCH_ID,
        twitchAppSecret: env.TWITCH_SECRET,
      })
  ...
  describe('Test global emotes (GraphQL)', () => {
    const emoteFetcher = new EmoteFetcher()
    const emoteParser = new EmoteParser(emoteFetcher, {
      type: 'markdown',
      match: /:(.+?):/g,
    ...
    test('Execute fetchTwitchEmotes with the GraphQL API', async () => {
      expect(await emoteFetcher.fetchTwitchEmotes(null, { api: 'gql' })).toBeInstanceOf(Collection)
    })
  ...
  describe('Test user emotes (GraphQL)', () => {
    const emoteFetcher = new EmoteFetcher()
    const emoteParser = new EmoteParser(emoteFetcher, {
      type: 'markdown',
      match: /:(.+?):/g,
    ...
    test('Execute fetchTwitchEmotes with user ID and the GraphQL API', async () => {
      expect(await emoteFetcher.fetchTwitchEmotes(56648155, { api: 'gql' })).toBeInstanceOf(Collection)
    })
